...
const PropertyDetails = ({ propertyInfo: { coverPhoto, price, rentFrequency, rooms, title, baths, area, agency, isVerified, description, type, purpose, amenities, photos, furnishingStatus } }) => {

    //  amenities.map(item => item.amenities.map(amenity => console.log(amenity)));
    //  amenities.map(item => item.amenities.map(amenity => amenity));
